...
```
`numfds`为要监视的文件描述符的最大值加1。然后分别为读、写、异常文件描述符集。`timeout`为超时，结构体中有秒和微秒的设置。


下面是一个select()实现多路复用的具体例子。通过select()函数来监听3个终端的输入，分别为：两个用mknod命令创建的管道（mknod 管道名 p）、主程序运行的终端。主程序终端的输入用来控制程序结束，如果是管道的输入则打印在屏幕上。

`注意：`在每一次运行select()函数之后，如果要再次运行select()都要重新设置文件描述符集，即调用FD_ZERO()和FD_SET()进行重新设置。为了方便起见，程序中一般会使用备用的描述符集。
...
```
numfds需要监听的文件个数。timeout阻塞超时时间单位为毫秒。返回revents中文件描述符的个数，0：超时，-1：出错。
...
`events：` POLLIN：文件有数据可读；POLLPRI：有紧急数据可读；POLLOUT：可以向文件写入数据；POLLERR：文件出现错误；POLLHUP：与文件的连接被断开；POLLNVAL：文件描述符不合法。
...
串口的配置一般包含：波特率、数据位、奇偶校验、停止位。下面的uart_api.c文件实现了两个函数，一个是打开串口的函数open_port()，一个是配置串口的函数set_com_config()。在实际使用中，只需要在头文件中包含uart_api.h就可以调用这两个函数来打开和配置串口了。
...
下面的程序使用select()通过多路复用的方式实现对串口的读写。可以在两台电脑上编译运行进行测试，还可以交叉编译后使用开发板和电脑进行测试。
