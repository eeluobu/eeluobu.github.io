...
void Dijkstra(int v0)
{
    ...
    for(int i = 0; i < n; i++)
    {
        int min = MAXINT;
        int u = v0;
        for(int j = 0; j < n; j++)   //不在S中的而且距离v0最小的结点u
        {
            if((!S[j]) && dis[j] < min)
            {
                u = j;
        ...
        for(int j = 0; j < n; j++)    //剩下的不在S中的结点v，更新dis[v] = min(dis[u]+dis[u][v], dis[v])
        {
            /*
            if((!S[j]) && dis[u] + a[u][j] < dis[j])  //防止a[u][j]溢出，要加一个判断
            {
                dis[j] = dis[u] + a[u][j];
                pre[j] = u;
            }
